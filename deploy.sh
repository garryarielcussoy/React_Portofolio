...
sudo docker build -f "Dockerfile" -t garryarielcussoy/serbabuku:fe-$TRAVIS_BUILD_NUMBER  --label "Maintaner  Garry Ariel <garry@alterra.id>" .
# push apps image to docker hub
sudo docker push garryarielcussoy/serbabuku:fe-$TRAVIS_BUILD_NUMBER
...
kubectl -n alta8 --kubeconfig kubeconfig set image deployment/${APP_K8S_DEPLOYMENT_NAME} ${APP_K8S_CONTAINER_NAME}=garryarielcussoy/serbabuku:fe-${TRAVIS_BUILD_NUMBER}